 ...
 */
public class DtLiveAnimController implements CommonPopupWindow.ViewInterface {
    private static final String TAG = "BaccaratLiveAnimControl";
    private Context mContext;
    private static DtLiveAnimController instance = null;
...
//    }
    ...
    public void init(Context context) {
        ...
        layoutInflater = ((DtLiveActivity) mContext).getWindow().getLayoutInflater();
    }
...
//                    initDefaultBackground();
    ...
    //---------------------daniu end------------------------
    public static DtLiveAnimController getInstance() {
        if (null == instance) {
            instance = new DtLiveAnimController();
        }
        return instance;
    }
...
//        popupWindow.showAsDropDown(v, 0, -(popupWindow.getHeight() + v.getMeasuredHeight()));
    ...
    private void downPopupWindow(View v) {
        if (popupWindow != null && popupWindow.isShowing()) return;
        popupWindow = new CommonPopupWindow.Builder(mContext)
                .setView(R.layout.pop_dt_limit)
                .setWidthAndHeight(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT)
                .setAnimationStyle(R.style.AnimDown)
                .setOutsideTouchable(true)
                .create();

        TextView tv_table_limit_dt = (TextView) popupWindow.getView().findViewById(R.id.tv_table_limit_dt);
        TextView tv_table_limit_tie = (TextView) popupWindow.getView().findViewById(R.id.tv_table_limit_tie);
        TextView tv_bet_limit_dt = (TextView) popupWindow.getView().findViewById(R.id.tv_bet_limit_dt);
        ...
        if (!TextUtils.isEmpty(SharedPreUtil.getInstance(mContext).getString(Constant.MIN_BET))) {
            ...
            String dt = String.valueOf(minBetInt[0]) + "-" + String.valueOf(maxBetInt[0]);
            String tie = String.valueOf(minBetInt[1]) + "-" + String.valueOf(maxBetInt[1]);
            String bet_dt = String.valueOf(betLimitInt[0]);
            String bet_tie = String.valueOf(betLimitInt[1]);
            tv_table_limit_dt.setText(dt);
            tv_table_limit_tie.setText(tie);
            tv_bet_limit_dt.setText(bet_dt);
    ...
    private void hideInTimeLayout() {
        ...
        if (isExpand == 0) {
            if (displayMetrics.widthPixels > 2100) {
                DtLiveInTimeController.getInstance().slideView(0, -(ScreenUtil.dip2px(315f)), rl_in_time_layout);//华为M5平板 676
            } else
                DtLiveInTimeController.getInstance().slideView(0, -(ScreenUtil.dip2px(236f)), rl_in_time_layout);//本来是向右平移，结果右边为负数，就以看到的点为坐标原点，向左平移
            iv_in.setRotation(0);
