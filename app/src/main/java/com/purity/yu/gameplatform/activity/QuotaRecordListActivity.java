...
import com.purity.yu.gameplatform.adapter.QuotaRecordAdapter;
 ...
 */
@ContentView(R.layout.activity_quota_record_list)
public class QuotaRecordListActivity extends BaseListActivity implements CommonPopupWindow.ViewInterface {
    ...
    private QuotaRecordAdapter adapter;
    private List<QuotaRecords> recordList;
    private List<QuotaRecords> recordListAll;
    ...
    private void initAdapter() {
        adapter = new QuotaRecordAdapter(mContext, recordListAll);
    ...
    protected void requestData(final int page) {
        postBetRecord(dateType, page);
    }

    private void postBetRecord(final int type, final int page) {
        String startDate = "";
        String endDate = "";
        ...
        HttpRequest.request(SharedPreUtil.getInstance(mContext).getString(ServiceIpConstant.BASE) + Constant.QUOTA_RECORD + "?token=" + token + "&startDate=" + startDate + "&endDate=" + endDate + "&page=" + page)
