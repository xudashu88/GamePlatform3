 ...
 */
public class BaccaratLiveAnimController implements CommonPopupWindow.ViewInterface {
    private static final String TAG = "视频切换";
    private Context mContext;
    private static BaccaratLiveAnimController instance = null;
...
//    }
    ...
    public void init(Context context) {
        ...
        layoutInflater = ((BaccaratLiveActivity) mContext).getWindow().getLayoutInflater();
    }
    ...
    //---------------------daniu end------------------------
    public static BaccaratLiveAnimController getInstance() {
        if (null == instance) {
            instance = new BaccaratLiveAnimController();
        }
        return instance;
    }
...
//        popupWindow.showAsDropDown(v, 0, -(popupWindow.getHeight() + v.getMeasuredHeight()));
    ...
    private void downPopupWindow(View v) {
        if (popupWindow != null && popupWindow.isShowing()) return;
        popupWindow = new CommonPopupWindow.Builder(mContext)
                .setView(R.layout.pop_bac_limit)
                .setWidthAndHeight(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT)
                .setAnimationStyle(R.style.AnimDown)
                .setOutsideTouchable(true)
                .create();

        TextView tv_table_limit_bp = (TextView) popupWindow.getView().findViewById(R.id.tv_table_limit_bp);
        TextView tv_table_limit_pair = (TextView) popupWindow.getView().findViewById(R.id.tv_table_limit_pair);
        TextView tv_table_limit_tie = (TextView) popupWindow.getView().findViewById(R.id.tv_table_limit_tie);
        TextView tv_table_limit_bs = (TextView) popupWindow.getView().findViewById(R.id.tv_table_limit_bs);

        TextView tv_bet_limit_bp = (TextView) popupWindow.getView().findViewById(R.id.tv_bet_limit_bp);
        TextView tv_bet_limit_tie = (TextView) popupWindow.getView().findViewById(R.id.tv_bet_limit_tie);
        TextView tv_bet_limit_pair = (TextView) popupWindow.getView().findViewById(R.id.tv_bet_limit_pair);
        TextView tv_bet_limit_bs = (TextView) popupWindow.getView().findViewById(R.id.tv_bet_limit_bs);
        ...
        if (!TextUtils.isEmpty(SharedPreUtil.getInstance(mContext).getString(Constant.MIN_BET))) {
            ...
            String bp = String.valueOf(minBetInt[0]) + "-" + String.valueOf(maxBetInt[0]);
            String tie = String.valueOf(minBetInt[1]) + "-" + String.valueOf(maxBetInt[1]);
            String pair = String.valueOf(minBetInt[2]) + "-" + String.valueOf(maxBetInt[2]);
            String bs = String.valueOf(minBetInt[3]) + "-" + String.valueOf(maxBetInt[3]);
            String bet_bp = String.valueOf(betLimitInt[0]);
            String bet_tie = String.valueOf(betLimitInt[1]);
            String bet_pair = String.valueOf(betLimitInt[2]);
            String bet_bs = String.valueOf(betLimitInt[3]);
            tv_bet_limit_bp.setText(bet_bp);
            tv_bet_limit_tie.setText(bet_tie);
            tv_bet_limit_pair.setText(bet_pair);
            tv_bet_limit_bs.setText(bet_bs);

            tv_table_limit_bp.setText(bp);
            tv_table_limit_pair.setText(pair);
            tv_table_limit_tie.setText(tie);
            tv_table_limit_bs.setText(bs);
        }
        popupWindow.showAsDropDown(v);
    }
    ...
    private void hideInTimeLayout() {
        ...
        if (isExpand == 0) {
            if (displayMetrics.widthPixels > 2100) {
                BaccaratLiveInTimeController.getInstance().slideView(0, -(ScreenUtil.dip2px(315f)), rl_in_time_layout);//华为M5平板 676
            } else
                BaccaratLiveInTimeController.getInstance().slideView(0, -(ScreenUtil.dip2px(236f)), rl_in_time_layout);//本来是向右平移，结果右边为负数，就以看到的点为坐标原点，向左平移
            iv_in.setRotation(0);
