...
public class BaccaratRVAdapter2 extends RecyclerView.Adapter<BaccaratRVAdapter2.ViewHolder> {
    ...
    public BaccaratRVAdapter2(Context context, List<Baccarat> baccaratList) {
    ...
    @Override
    public BaccaratRVAdapter2.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_baccarat, parent, false);
        return new BaccaratRVAdapter2.ViewHolder(v);
    }
    ...
    @Override
    public void onBindViewHolder(final BaccaratRVAdapter2.ViewHolder holder, final int position) {
        scale(holder);
...
//                ((BaccaratListActivity) mContext).finish();
    ...
    private void init(ViewHolder holder, int position, int times) {
        times++;
        LogUtil.i("百家乐第一次进去次数times=" + times);
