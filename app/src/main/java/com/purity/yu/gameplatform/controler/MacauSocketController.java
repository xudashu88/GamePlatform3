 ...
 */
public class MacauSocketController {
    private Context mContext;
    private static MacauSocketController instance = null;
...
//            LogUtil.i("SOCKET_PRODICT" + SharedPreUtil.getInstance(mContext).getString(ServiceIpConstant.SOCKET_PRODICT));
...
//    }
    ...
    public static MacauSocketController getInstance() {
        if (null == instance) {
            instance = new MacauSocketController();
        }
        return instance;
    }
...
//                                maxScoreList = BaccaratUtil.getInstance().initMaxScore(56, maxScoreList);
                                    postEventJoinRoom(boardMessageList, maxScoreList, _goldcoins, roomid, roomName, _betLimitStr, _maxBetStr, _minBetStr, stateHis, downtownTime, skyCard, isPeopleNum, betSecond, betsRemind);
    ...
    private void postEventJoinRoom(List<Integer> boardMessageList, List<Integer> maxScoreList, String _goldCoins, String roomId, String roomName,
                                   String[] betLimitList, String[] maxBetList, String[] minBetList, String state, int downtownTime, int skyCard, int isPeopleNum, int betSecond, int betsRemind) {
        ObjectEvent.BoardMessageEvent event = new ObjectEvent.BoardMessageEvent();
        event.boardMessageList = boardMessageList;
        event.maxScoreList = maxScoreList;
