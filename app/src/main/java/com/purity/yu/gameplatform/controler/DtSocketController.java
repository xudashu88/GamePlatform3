 ...
 */
public class DtSocketController {
    ...
    public static DtSocketController getInstance() {
        if (null == instance) {
            instance = new DtSocketController();
        }
        return instance;
    }

    public void copyWidget(List<Integer> boardMessageList, TextView tv_time, RelativeLayout ll_poker, ImageView iv_jet_banker_01,
                           TextView tv_jet_banker_value, ImageView iv_jet_player_01, TextView tv_jet_player_value, String roomId,
                        ...
                        switch (command) {
                            ...
                            case "dobet"://下注成功
                                try {
                                    ...
                                    if (bankScore != 0) {
                                        String _bank = mContext.getResources().getString(R.string.dragon);
                                        sb.append(_bank + ":" + bankScore + " " + _bet_successful + "\n");
                                    }
                                    if (playScore != 0) {
                                        String _play = mContext.getResources().getString(R.string.tiger);
...
//                                    maxScoreList.add(0);
    ...
    private void postEventJoinRoom(List<Integer> boardMessageList, List<Integer> maxScoreList, String _goldCoins, String roomId, String roomName,
                                   String[] betLimitList, String[] maxBetList, String[] minBetList, String state, int downtownTime, int skyCard, int isPeopleNum, int betSecond, String[] _colorStr, int betsRemind) {
        ObjectEvent.BoardMessageEvent event = new ObjectEvent.BoardMessageEvent();
        event.boardMessageList = boardMessageList;
        event.maxScoreList = maxScoreList;
